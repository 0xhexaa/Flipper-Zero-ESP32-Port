...
#define TAG "AirplayWifi"
/* Worker issues esp_wifi/lwIP calls. 4096 words (16 KB), matching wlan_hal. */
#define AIRPLAY_WIFI_WORKER_STACK 4096
...
static bool airplay_reason_is_auth(uint8_t reason) {
    switch(reason) {
...
static void airplay_event_handler(
    void* arg,
    ...
    if(event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_DISCONNECTED) {
        ...
        if(d) {
            ...
            if(airplay_reason_is_auth((uint8_t)d->reason)) s_auth_fail_latched = true;
        }
...
static void airplay_worker_fn(void* arg) {
    UNUSED(arg);
    ...
    while(1) {
        ...
        case ACMD_INIT_START:
            ...
            if(!s_event_handlers_registered) {
                esp_event_handler_register(
                    WIFI_EVENT, ESP_EVENT_ANY_ID, &airplay_event_handler, NULL);
                esp_event_handler_register(
                    IP_EVENT, IP_EVENT_STA_GOT_IP, &airplay_event_handler, NULL);
        ...
        case ACMD_STOP_DEINIT:
            ...
            esp_wifi_stop();
            esp_wifi_deinit();
            /* Deregister the event handlers — airplay_event_handler lives in the
            ...
            if(s_event_handlers_registered) {
                esp_event_handler_unregister(
                    WIFI_EVENT, ESP_EVENT_ANY_ID, &airplay_event_handler);
                esp_event_handler_unregister(
                    IP_EVENT, IP_EVENT_STA_GOT_IP, &airplay_event_handler);
...
static bool airplay_ensure_worker(void) {
    if(s_worker_task) return true;

    s_cmd_queue = xQueueCreate(4, sizeof(AirplayCmd));
    if(!s_cmd_queue) return false;

    s_worker_stack = heap_caps_malloc(
        AIRPLAY_WIFI_WORKER_STACK * sizeof(StackType_t), MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
    ...
    s_worker_task = xTaskCreateStaticPinnedToCore(
        airplay_worker_fn, "AirplayWifi", AIRPLAY_WIFI_WORKER_STACK,
        NULL, 5, s_worker_stack, s_worker_buf, 0);
    return s_worker_task != NULL;
}

static void airplay_send_cmd_sync(AirplayCmd* cmd) {
...
bool airplay_wifi_start(void) {
    ...
    if(!airplay_ensure_worker()) return false;

    volatile bool result = false;
    AirplayCmd cmd = {.type = ACMD_INIT_START, .result = &result};
    airplay_send_cmd_sync(&cmd);
...
void airplay_wifi_stop(void) {
    if(s_started) {
        AirplayCmd cmd = {.type = ACMD_STOP_DEINIT};
        airplay_send_cmd_sync(&cmd);
        s_started = false;
        ESP_LOGI(TAG, "WiFi stopped");
    }

    /* Terminate the worker task. It runs airplay_worker_fn() from the FAP text
    ...
    if(s_worker_task) {
        volatile bool done = false;
        AirplayCmd cmd = {.type = ACMD_QUIT, .done = &done};
        xQueueSend(s_cmd_queue, &cmd, portMAX_DELAY);
...
bool airplay_wifi_connect(
    ...
    AirplayCmd cmd = {.type = ACMD_CONNECT};
...
void airplay_wifi_disconnect(void) {
    if(!s_started) return;
    AirplayCmd cmd = {.type = ACMD_DISCONNECT};
    airplay_send_cmd_sync(&cmd);
}
...
bool airplay_wifi_run_in_worker(AirplayWifiWorkerFn fn, void* arg) {
    ...
    AirplayCmd cmd = {.type = ACMD_RUN_FN, .run_fn = {.fn = fn, .arg = arg}};
    airplay_send_cmd_sync(&cmd);
    return true;
}
